// Loads gameModes/trivia/content/seed/*.json into Supabase.
// Usage: node --env-file=.env.local scripts/seedTrivia.mts [file ...]
...
if (!supabaseUrl || !serviceRoleKey) {
  throw new Error(
    'Missing NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY — run with `node --env-file=.env.local scripts/seedTrivia.mts`'
  );
}

const supabase = createClient(supabaseUrl, serviceRoleKey);
const seedDir = path.join(path.dirname(fileURLToPath(import.meta.url)), '../gameModes/trivia/content/seed');
...
async function seedTrack(fileName: string) {
  ...
  const rows = track.questions.map((question, index) => ({
    ...
    grammar_point: question.grammarPoint,
    time_limit_seconds: question.timeLimitSeconds,
  }));

  const { error: questionsError } = await supabase.from('trivia_questions').insert(rows);
  if (questionsError) throw questionsError;

  console.log(`Seeded ${track.level}: ${track.title} (${rows.length} questions)`);
}
